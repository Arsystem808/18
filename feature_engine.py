...
def pivots_daily(df:pd.DataFrame)->pd.DataFrame:
    ...
    h1=df['high'].shift(1); l1=df['low'].shift(1); c1=df['close'].shift(1)
    P=(h1+l1+c1)/3.0
    R1=2*P-l1; S1=2*P-h1; R2=P+(h1-l1); S2=P-(h1-l1)
    piv['pivot']=P; piv['r1']=R1; piv['s1']=S1; piv['r2']=R2; piv['s2']=S2
    return piv

def build_features(df:pd.DataFrame)->pd.DataFrame:
    out=df.copy(); out['ret1']=out['close'].pct_change(); out['atr14']=atr(out,14); out['mom10']=momentum(out,10)
    piv=pivots_daily(out); out=out.join(piv)
    out['dist_pivot']=(out['close']-out['pivot'])/out['close']
    out['dist_r1']=(out['r1']-out['close'])/out['close']
    out['dist_s1']=(out['close']-out['s1'])/out['close']
    out=out.dropna(); return out
